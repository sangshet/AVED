...
def main(args):
    ...
    try:
        ...
        with open(os.path.abspath(os.path.join(SCRIPT_DIR, 'package_data', 'prerm.sh')), 'r') as infile:
            fdata = infile.read()
            fdata = fdata.replace('#!/bin/sh',                  '')
            fdata = fdata.replace('MODULE_NAME=$1',             'MODULE_NAME='+ config['package']['name'])
            fdata = fdata.replace('MODULE_VERSION_STRING=$2',   'MODULE_VERSION_STRING='+ config['package']['version'])
        ...
        with open(os.path.abspath(os.path.join(SCRIPT_DIR, 'package_data', 'preinst.sh')), 'r') as infile:
            fdata = infile.read()
            fdata = fdata.replace('#!/bin/sh',                  '')
            fdata = fdata.replace('MODULE_NAME=$1',             'MODULE_NAME='+ config['package']['name'])
            fdata = fdata.replace('MODULE_VERSION_STRING=$2',   'MODULE_VERSION_STRING='+ config['package']['version'])
        ...
        check_output_file_exists('BUILD_AMI_TOOL', join(PROJECT_DIR, 'app', 'build', 'ami_tool'))
        ...
        driver_dest = [
            {
                'src': os.path.abspath(join(PROJECT_DIR, f)),
